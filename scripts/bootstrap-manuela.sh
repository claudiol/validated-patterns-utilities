...
openShiftLogin()
{
  OCP_SERVER=$(oc project default | awk '{print $7}' | sed "s/\"\./\"/g" | sed "s/\"//g") 
...
{
  REG_HOST=$(oc get route default-route -n openshift-image-registry --template='{{ .spec.host }}')
  LINE_IMAGE_EXISTS=$(oc image info ${REG_HOST}/manuela-tst-all/line-dashboard:0.3.1-81 --insecure -a /run/user/1000/containers/auth.json | grep Digest | awk '{print $2}')
  if [ "$LINE_IMAGE_EXISTS." == "." ]; then
    echo -n "Boostrapping Manuela image [line-dashboard] ..."
    oc image mirror quay.io/claudiol/iot-frontend:0.3.1-81 ${REG_HOST}/manuela-tst-all/line-dashboard:0.3.1-81 --insecure=true -a /run/user/1000/containers/auth.json
    echo "done"
  else
    echo "Line Dashboard image exists [$LINE_IMAGE_EXISTS]"
  fi

  MACHINE_IMAGE_EXISTS=$(oc image info ${REG_HOST}/manuela-tst-all/machine-sensor:0.3.1-56 --insecure -a /run/user/1000/containers/auth.json | grep Digest | awk '{print $2}')
  if [ "$MACHINE_IMAGE_EXISTS." == "." ]; then
    echo -n "Boostrapping Manuela image [machine-sensor] ..."
    oc image mirror quay.io/claudiol/iot-software-sensor:0.3.1-56 ${REG_HOST}/manuela-tst-all/machine-sensor:0.3.1-56 --insecure=true -a /run/user/1000/containers/auth.json
    echo "done"
  else
    echo "Machine Sensor image exists [$MACHINE_IMAGE_EXISTS]"
  fi

  MESSAGING_IMAGE_EXISTS=$(oc image info ${REG_HOST}/manuela-tst-all/messaging:0.3.2-87 --insecure -a /run/user/1000/containers/auth.json | grep Digest | awk '{print $2}')
  if [ "$MESSAGING_IMAGE_EXISTS." == "." ]; then
    echo -n "Boostrapping Manuela image [messaging] ..."
    oc image mirror quay.io/claudiol/iot-consumer:0.3.2-87 ${REG_HOST}/manuela-tst-all/messaging:0.3.2-87 --insecure=true -a /run/user/1000/containers/auth.json
    echo "done"
  else
